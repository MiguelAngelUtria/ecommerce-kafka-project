// services/notification-service/config/kafkaClient.js
    ...
    try {
        ...
        await consumer.run({
             eachMessage: async ({ topic, partition, message }) => {
                 // console.log(`Received message offset ${message.offset} from topic ${topic}`);
                 try {
                    // El mensaje contiene el EventData completo enviado por el servicio anterior (welcome, cart, etc.)
                    const messageValue = JSON.parse(message.value.toString());
                    await eachMessageHandler(messageValue);
                 } catch (error) {
